...
clear
echo ""
echo -e "  ${BOLD}Teo's Dev Environment${NC}"
echo ""
echo -e "  A curated terminal setup: Ghostty, modern CLI replacements,"
echo -e "  language runtimes, and Claude Code (AI coding agent)."
echo ""
...
brew_install "zsh-autosuggestions" "ghost text suggestions as you type (from history)"
brew_install "zsh-syntax-highlighting" "colors commands green/red as you type"
...
echo ""
echo -e "  You now have a curated dev environment — terminal, git tools,"
echo -e "  language runtimes, and an AI coding agent."
echo ""
echo -e "  You also have ${BOLD}nexus${NC}, which is different from everything else"
echo -e "  that was just installed. It's invisible project infrastructure."
echo ""
echo -e "  ${BOLD}How it works${NC}"
echo ""
echo -e "  ${GREEN}1.${NC} Start or enter a project:  ${DIM}mkdir myapp && cd myapp${NC}"
echo -e "  ${GREEN}2.${NC} Run:                        ${DIM}nexus init${NC}"
echo -e "  ${GREEN}3.${NC} Code like normal."
echo ""
echo -e "  That's it. Behind the scenes, nexus:"
echo -e "    Detects your stack ${DIM}(Node, Go, Python, Rust)${NC}"
echo -e "    Validates env vars and imports on every commit"
echo -e "    Keeps AI context fresh across Claude Code sessions"
echo -e "    Catches drift when a session ends"
echo ""
echo -e "  No config files to maintain. No commands to remember."
echo -e "  You just code — nexus handles the plumbing."
echo ""
echo -e "  ${BOLD}Next steps${NC}"
echo ""
NEXT_STEP=1
echo "  ${NEXT_STEP}. Restart your terminal ${DIM}(or run: source ~/.zshrc)${NC}"
NEXT_STEP=$((NEXT_STEP + 1))
if has_cask docker 2>/dev/null; then
  echo "  ${NEXT_STEP}. Open Docker Desktop ${DIM}(first launch needs manual start)${NC}"
  NEXT_STEP=$((NEXT_STEP + 1))
fi
if has_cmd claude; then
  echo "  ${NEXT_STEP}. Run ${BOLD}claude${NC} to sign in to your Anthropic account"
  NEXT_STEP=$((NEXT_STEP + 1))
fi
echo "  ${NEXT_STEP}. Open a project and run ${BOLD}nexus init${NC}"
echo ""
...
echo -e "  ${CYAN}Split pane${NC}  Cmd+D            ${CYAN}Man Pages${NC}   tldr"
echo ""
echo -e "  ${DIM}Run${NC} ${BOLD}nexus readme${NC} ${DIM}anytime for the full breakdown.${NC}"
echo ""

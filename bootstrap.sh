...
show_audit() {
  {
  echo ""
  echo -e "${BOLD}Everything this script installs${NC}"
  echo -e "${DIM}Items marked ${YELLOW}optional${NC}${DIM} will ask before installing.${NC}"
  echo -e "${DIM}Already-installed tools are skipped automatically.${NC}"
  echo ""
  echo -e "${BOLD}Prerequisites${NC}"
  echo -e "  ${CYAN}xcode-select${NC}    git, clang, make — needed by everything"
  echo -e "  ${CYAN}homebrew${NC}        macOS package manager (brew.sh)"
  echo ""
  echo -e "${BOLD}Terminal${NC}"
  echo -e "  ${CYAN}ghostty${NC}         GPU-accelerated terminal written in Zig (ghostty.org)"
  echo -e "  ${CYAN}starship${NC}        Fast cross-shell prompt written in Rust (starship.rs)"
  echo -e "  ${CYAN}JetBrains Mono${NC}  Nerd Font with ligatures + icons"
  echo ""
  echo -e "${BOLD}Modern CLI Replacements${NC}"
  echo -e "  ${CYAN}ripgrep${NC}         grep but 100x faster (github.com/BurntSushi/ripgrep)"
  echo -e "  ${CYAN}bat${NC}             cat with syntax highlighting (github.com/sharkdp/bat)"
  echo -e "  ${CYAN}eza${NC}             ls with icons + git status (github.com/eza-community/eza)"
  echo -e "  ${CYAN}fd${NC}              find but simpler + faster (github.com/sharkdp/fd)"
  echo -e "  ${CYAN}zoxide${NC}          Smart cd, learns your dirs (github.com/ajeetdsouza/zoxide)"
  echo -e "  ${CYAN}fzf${NC}             Fuzzy finder for everything (github.com/junegunn/fzf)"
  echo -e "  ${CYAN}git-delta${NC}       Syntax-highlighted git diffs (github.com/dandavison/delta)"
  echo -e "  ${CYAN}lazygit${NC}         Terminal UI for git (github.com/jesseduffield/lazygit)"
  echo ""
  echo -e "${BOLD}Advanced Tools${NC}"
  echo -e "  ${CYAN}atuin${NC}           Shell history search + sync (atuin.sh)"
  echo -e "  ${CYAN}httpie${NC}          Human-friendly HTTP client (httpie.io)"
  echo -e "  ${CYAN}jq${NC}              JSON processor (jqlang.github.io)"
  echo -e "  ${CYAN}yazi${NC}            Terminal file manager (github.com/sxyazi/yazi)"
  echo -e "  ${CYAN}hyperfine${NC}       CLI benchmarking (github.com/sharkdp/hyperfine)"
  echo -e "  ${CYAN}lazydocker${NC}      Terminal UI for Docker (github.com/jesseduffield/lazydocker)"
  echo -e "  ${CYAN}tldr${NC}            Simplified man pages (tldr.sh)"
  echo -e "  ${CYAN}btop${NC}            System monitor (github.com/aristocratos/btop)"
  echo -e "  ${CYAN}dust${NC}            Disk usage analyzer (github.com/bootandy/dust)"
  echo -e "  ${CYAN}dive${NC}            Docker image layer inspector (github.com/wagoodman/dive)"
  echo -e "  ${CYAN}gh${NC}              GitHub CLI (cli.github.com)"
  echo -e "  ${CYAN}just${NC}            Command runner, replaces make (github.com/casey/just)"
  echo -e "  ${CYAN}raycast${NC}         App launcher + clipboard + windows (raycast.com) ${YELLOW}optional${NC}"
  echo ""
  echo -e "${BOLD}Runtimes & Package Managers${NC}"
  echo -e "  ${CYAN}mise${NC}            Version manager for all runtimes (mise.jdx.dev)"
  echo -e "  ${CYAN}node 24 LTS${NC}     JavaScript runtime (via mise)"
  echo -e "  ${CYAN}pnpm${NC}            Fast, strict JS package manager (pnpm.io)"
  echo -e "  ${CYAN}bun${NC}             Fast JS runtime + bundler (bun.sh) ${YELLOW}optional${NC}"
  echo -e "  ${CYAN}python 3.12${NC}     Python runtime (via mise) ${YELLOW}optional${NC}"
  echo -e "  ${CYAN}uv${NC}              Fast Python package manager (astral.sh) ${YELLOW}optional${NC}"
  echo -e "  ${CYAN}ruff${NC}            Fast Python linter (astral.sh) ${YELLOW}optional${NC}"
  echo ""
  echo -e "${BOLD}Applications${NC}"
  echo -e "  ${CYAN}docker${NC}          Containerization platform (docker.com) ${YELLOW}optional${NC}"
  echo -e "  ${CYAN}claude-code${NC}     CLI AI coding agent (anthropic.com)"
  echo -e "  ${CYAN}nexus${NC}           Session launcher CLI (this repo)"
  echo ""
  echo -e "${BOLD}Config files written${NC} ${DIM}(only if they don't already exist)${NC}"
  echo -e "  ${CYAN}~/.config/ghostty/config${NC}"
  echo -e "    Catppuccin Mocha theme, transparency, splits, hidden titlebar"
  echo -e "  ${CYAN}~/.config/starship.toml${NC}"
  echo -e "    Minimal prompt, gcloud/aws disabled"
  echo -e "  ${CYAN}~/.config/mise/config.toml${NC}"
  echo -e "    node@24, python@3.12, auto_install enabled"
  echo -e "  ${CYAN}~/.gitconfig${NC} ${DIM}(via git config)${NC}"
  echo -e "    delta pager, rebase on pull, rerere, side-by-side diffs"
  echo -e "  ${CYAN}~/.zshrc${NC} ${DIM}(appended, not overwritten)${NC}"
  echo -e "    Tool integrations, aliases, yazi function"
  echo ""
  echo -e "${DIM}Source: https://github.com/teohondascully/nexus/blob/main/bootstrap.sh${NC}"
  echo ""
  } | less -R
}
...
echo -e "${DIM}Last approved by Teo: 2026-04-11${NC}"
echo ""
echo "This script sets up a complete dev environment:"
echo ""
echo -e "  ${CYAN} 1${NC}  Xcode CLI Tools      ${DIM}git, compilers${NC}"
echo -e "  ${CYAN} 2${NC}  Homebrew              ${DIM}package manager${NC}"
echo -e "  ${CYAN} 3${NC}  Ghostty + config      ${DIM}terminal emulator${NC}"
echo -e "  ${CYAN} 4${NC}  Modern CLI tools      ${DIM}ripgrep, bat, eza, etc.${NC}"
echo -e "  ${CYAN} 5${NC}  Advanced CLI tools    ${DIM}httpie, yazi, btop, etc.${NC}"
echo -e "  ${CYAN} 6${NC}  mise                  ${DIM}version manager${NC}"
echo -e "  ${CYAN} 7${NC}  Node + pnpm + Bun     ${DIM}JS runtimes & packages${NC}"
echo -e "  ${CYAN} 8${NC}  Python + uv + Ruff    ${DIM}Python toolchain${NC}"
echo -e "  ${CYAN} 9${NC}  Docker Desktop        ${DIM}containerization${NC}"
echo -e "  ${CYAN}10${NC}  Claude Code           ${DIM}CLI AI agent${NC}"
echo -e "  ${CYAN}11${NC}  Git config            ${DIM}delta, rebase, rerere${NC}"
echo -e "  ${CYAN}12${NC}  Shell config          ${DIM}aliases, prompt, tools${NC}"
echo ""
echo -e "${GREEN}Safe to re-run${NC} — skips anything already installed."
echo -e "${YELLOW}Optional steps${NC} will ask before installing."
echo ""
echo -e "  ${BOLD}Enter${NC}  Start install"
echo -e "  ${BOLD}i${NC}      Inspect full list of tools, sources, and configs"
...
case $REPLY in
  i|I) show_audit
       echo -e "  ${BOLD}Enter${NC} to continue with install, ${BOLD}q${NC} to quit"
       read -n 1 -r
       echo ""

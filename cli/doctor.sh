...
cmd_doctor() {
  ...
  if [ -f "CLAUDE.md" ] && grep -q "nexus:file-structure" "CLAUDE.md" 2>/dev/null && [ -f "scripts/sync-claude-md.sh" ]; then
    ...
    backup=$(mktemp)
    cp "CLAUDE.md" "$backup"
    bash scripts/sync-claude-md.sh > /dev/null 2>&1 || true
  ...
  if $fix && [ ! -f ".env.example" ]; then
    ...
    ((passed++)) || true
  elif [ -f "scripts/check-env-sync.sh" ]; then
    local env_output
  ...
  if ! $quick; then
    ...
    # No linter
    if [ -f "package.json" ] && ! grep -qE '"eslint"|"biome"|"@biomejs"' package.json 2>/dev/null; then
    ...
    # No test runner
    if [ -f "package.json" ] && ! grep -qE '"vitest"|"jest"|"playwright"|"@testing-library"' package.json 2>/dev/null; then
    ...
    # CommonJS
    if [ -f "package.json" ] && ! grep -qE '"type":\s*"module"' package.json 2>/dev/null; then

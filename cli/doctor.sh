...
cmd_doctor() {
  ...
  if [ "$eco" != "generic" ]; then
    if $fix && [ ! -f ".env.example" ]; then
      ...
      # Use ecosystem-aware pattern matching (same logic as check-env-sync.sh)
      local fix_pattern="" fix_strip="" fix_includes=""
      case "$eco" in
        node)   fix_pattern='(process\.env|Bun\.env)\.[A-Z_][A-Z0-9_]*'; fix_strip='s/(process\.env|Bun\.env)\.//'; fix_includes='--include=*.ts --include=*.tsx --include=*.js --include=*.jsx' ;;
        go)     fix_pattern='os\.Getenv\("[A-Z_][A-Z0-9_]*"\)'; fix_strip='s/os\.Getenv\("//;s/"\)//'; fix_includes='--include=*.go' ;;
        python) fix_pattern='os\.environ\[?"[A-Z_][A-Z0-9_]*"\]?|os\.getenv\("[A-Z_][A-Z0-9_]*"\)'; fix_strip='s/os\.environ\[?"//;s/"\]?//;s/os\.getenv\("//;s/"\)//'; fix_includes='--include=*.py' ;;
        rust)   fix_pattern='std::env::var\("[A-Z_][A-Z0-9_]*"\)'; fix_strip='s/std::env::var\("//;s/"\)//'; fix_includes='--include=*.rs' ;;
      esac
      if [ -n "$fix_pattern" ]; then
        eval grep -roEh $fix_includes "'$fix_pattern'" . 2>/dev/null \
          | grep -oE "$fix_pattern" | sed -E "$fix_strip" | sort -u | while IFS= read -r var; do
            [ -n "$var" ] && echo "${var}=" >> .env.example
          done
      fi
  ...
  local fail_suffix=""
  [ "$failed" -gt 0 ] 2>/dev/null && fail_suffix=", ${failed} failed"
  echo "- Result: ${passed}/${total} passed${fail_suffix}" >> .nexus/.last-doctor
  if [ -n "${rec_lines:-}" ]; then
    local clean_recs
